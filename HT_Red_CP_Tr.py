...
CostoRac = 6000 # $/MWh
Pbase = 100 # MVA
max_ang = 60 # Maximo angulo de las barras (positivo)

abs_path = os.getcwd()
file = abs_path + '\\' + 'DataHT_39.xlsx'
...
df_Tramos       = pd.read_excel(file,sheet_name='Tramos')
...
for j in df_Red.index:
    ...
    df_Red.loc[j,'factor_Pl'] = df_Red.loc[j,'R']/(df_Red.loc[j,'R']*df_Red.loc[j,'R'] + df_Red.loc[j,'X']*df_Red.loc[j,'X'])  

df_Pendientes = pd.DataFrame(np.nan,index = df_Red['Linea'], columns = df_Tramos['Tramos'])

# df_Tramos.shape[0]

# for lin in df_Pendientes.index:
#     for trm in df_Pendientes.columns:
#         print()
#         df_Pendientes.loc[lin,trm] = max_ang/df_Tramos.shape[0]*(df_Red.loc[lin,'factor_Pl'])

# enumerate(df_Pendientes.index)
delta_ang_ref = max_ang/df_Tramos.shape[0]
# Recorrer las filas y columnas e imprimir la posición y el valor
for row_idx, lin in enumerate(df_Pendientes.index):
    for col_idx, trm in enumerate(df_Pendientes.columns):
        cell_value = df_Pendientes.at[lin, trm]  # Obtener el valor de la celda
        df_Pendientes.loc[lin,trm] = ((1-math.cos(delta_ang_ref*(col_idx+1)*math.pi/180))*(df_Red.loc[row_idx,'factor_Pl'])
                                    -(1-math.cos(delta_ang_ref*(col_idx)*math.pi/180))*(df_Red.loc[row_idx,'factor_Pl']))/(max_ang/df_Tramos.shape[0]*math.pi/180)
...
model.sTrm   = pyo.Set(initialize = df_Tramos['Tramos'].values.tolist())
...
for Emb in model.sEmb:
    for t in model.st:
        ...
        model.vV[Emb,t].setlb(df_Vmin.loc[getIndexes(df_Vmax, Emb)[0][0],t])
#model.vV.display()
...
model.vd.setlb(-1*max_ang)
model.vd.setub(max_ang)
# for t in model.st:
...
model.vd_s = pyo.Var(model.sLin, model.st, domain=pyo.NonNegativeReals, initialize=0)
model.vd_r = pyo.Var(model.sLin, model.st, domain=pyo.NonNegativeReals, initialize=0)

model.vd_sd = pyo.Var(model.sLin, model.st, model.sTrm, domain=pyo.NonNegativeReals, initialize=0)
model.vd_sd.setub(delta_ang_ref)
model.vd_rd = pyo.Var(model.sLin, model.st, model.sTrm, domain=pyo.NonNegativeReals, initialize=0)
model.vd_rd.setub(delta_ang_ref)
...
for Barra in model.sBarra:                                
    for t in model.st:
        model.eBalNod.add(  sum( model.vPt[Term,t] for Term in model.sTerm if (Term,Barra) in model.sMaptb) 
                          ...
                          + Pbase*sum(sum(sum(df_Pendientes.loc[lin,trm]*model.vd_sd[lin,t,trm]*math.pi/180 for trm in model.sTrm) for lin in model.sLin if ((lin,Barra,barra_par) or (lin,barra_par,Barra)) in model.sMaplbb) for barra_par in model.sBarra )
                          + Pbase*sum(sum(sum(df_Pendientes.loc[lin,trm]*model.vd_rd[lin,t,trm]*math.pi/180 for trm in model.sTrm) for lin in model.sLin if ((lin,Barra,barra_par) or (lin,barra_par,Barra)) in model.sMaplbb) for barra_par in model.sBarra )
...
# model.eBalNod.pprint()        


# Ecuación de modelado por tramos con pendientes crecientes
model.eAngLin_1 = pyo.ConstraintList()
for lin in model.sLin:
    for barra in model.sBarra:
        for barra_par in model.sBarra:
            if (lin,barra,barra_par) in model.sMaplbb:
                for t in model.st:
                    model.eAngLin_1.add(
                                    model.vd[barra,t] - model.vd[barra_par,t] == model.vd_s[lin,t] - model.vd_r[lin,t]                             
                                    )
                
#Diferencia angular discretizada de envío de la línea lin
model.eAngLin_2 = pyo.ConstraintList()
for lin in model.sLin:
    for t in model.st:
        model.eAngLin_2.add(
                        model.vd_s[lin,t] == sum( model.vd_sd[lin,t,trm] for trm in model.sTrm )                             
                        )

#Diferencia angular discretizada de recepción de la línea lin
model.eAngLin_3 = pyo.ConstraintList()
for lin in model.sLin:
    for t in model.st:
        model.eAngLin_3.add(
                        model.vd_r[lin,t] == sum( model.vd_rd[lin,t,trm] for trm in model.sTrm )                             
                        )
...
rdf_vd_s = pd.DataFrame(np.nan,index = model.st, columns = model.sLin )
for t in model.st:
    for Lin in model.sLin:
        rdf_vd_s.loc[t,Lin] = model.vd_s[Lin,t].value

rdf_vd_r = pd.DataFrame(np.nan,index = model.st, columns = model.sLin )
for t in model.st:
    for Lin in model.sLin:
        rdf_vd_r.loc[t,Lin] = model.vd_r[Lin,t].value
        
rdf_vd_sd ={}
for t in model.st:
    rdf_vd_sd[t] = pd.DataFrame(np.nan, index = model.sLin, columns = model.sTrm)
    for Lin in model.sLin:
        for Trm in model.sTrm:
            rdf_vd_sd[t].loc[Lin,Trm] = model.vd_sd[Lin,t,Trm].value
        
# =============================================================================
# SECCION PARA GRAFICOS
# =============================================================================
Texto = "Resultados_de_Simulación_Base"
