...
df_TurbEmb       = pd.read_excel(file,sheet_name='DTurbEmb')
...
model.sMapTurb = pyo.Set(initialize = df_TurbEmb[["CH","Embalse"]].values.tolist())
        ...
        if t == model.st.first():
            model.eBalEmb.add( model.vV[Emb,t] == df_DEmb.loc[getIndexes(df_DEmb, Emb)[0][0],'Vini'] 
                              + 3.6*( sum( df_CaudIn.loc[getIndexes(df_CaudIn, Caud)[0][0],t] for Caud in model.sCaud if (Emb,Caud) in model.sMapembq) 
                                     ...
                                     - model.vSp[Emb,t]
                                     + sum( model.vPh[Hidro,t]/(df_DHidro.loc[getIndexes(df_DHidro, Hidro)[0][0],'CoefProd']) for Hidro in model.sHidro if (Hidro,Emb) in model.sMapTurb )
                                     )
                              )
...
Texto = "Resultados_de_Simulacion_ConexcionEmb_1.0"
